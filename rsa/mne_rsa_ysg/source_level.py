...
def rsa_nifti(
    ...
    if rsa_metric == 'regression':
        rsa_result, r_squared = rsa_array(
            X,
            rdm_model,
            patches,
        ...
        if one_model:
            ...
            data[result_mask] = rsa_result[:, 0]  # 取出第一个RDM的权重
            r_squared_data = np.zeros(image.shape[:3])
            r_squared_data[result_mask] = r_squared
            return nib.Nifti1Image(data, image.affine, image.header), nib.Nifti1Image(r_squared_data, image.affine, image.header)
        else:
            results = []
            for i in range(rsa_result.shape[-1]):
                data = np.zeros(image.shape[:3])
                data[result_mask] = rsa_result[:, i]  # 分别保存每个RDM的权重
                r_squared_data = np.zeros(image.shape[:3])
                r_squared_data[result_mask] = r_squared  # 共享r_squared
                results.append(nib.Nifti1Image(data, image.affine, image.header))
            results.append(nib.Nifti1Image(r_squared_data, image.affine, image.header))
            return results
    if rsa_metric == 'partial-regression':
        results_pr = rsa_array(
            X,
            rdm_model,
            patches,
            data_rdm_metric=image_rdm_metric,
        ...
        #print(len(results_pr))
        results = []
        for i in range(len(results_pr)):
            data = np.zeros(image.shape[:3])
            data[result_mask] = results_pr[i].reshape(-1) # 分别保存每个RDM的权重
            #r_squared_data = np.zeros(image.shape[:3])
            #r_squared_data[result_mask] = r_squared  # 共享r_squared
            results.append(nib.Nifti1Image(data, image.affine, image.header))
        #results.append(nib.Nifti1Image(r_squared_data, image.affine, image.header))
